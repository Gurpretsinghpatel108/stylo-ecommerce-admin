...
import { GET_ALL_PRODUCTS_API, getImageUrl } from "../services/api"; // ✅ Yeh import sab fix karega
...
export default function ProductsScreen() {
  ...
  const loadCartCount = async () => {
    try {
      ...
      const qtyMap = {};
      data.forEach((item) => {
        if (item._id) qtyMap[item._id] = item.quantity || 0;
      });
  ...
  const loadWishlist = async () => {
    try {
      ...
      wishlist.forEach((item) => {
        if (item._id) favMap[item._id] = true;
      });
      setFavorites(favMap);
    } catch (err) {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Filter by category/subcategory from params
      if (categoryId) {
      ...
      if (subcategoryId) {
        allProducts = allProducts.filter(
          (p) => String(p.subcategoryId?._id) === String(subcategoryId)
        );
      }
      ...
      allProducts.forEach((p) => {
        if (p.subcategoryId?.name && !subs.includes(p.subcategoryId.name)) {
          subs.push(p.subcategoryId.name);
        }
      });
      setUniqueSubcategories(subs);

      // Auto-select subcategory if coming from params
      if (subcategoryName && subs.includes(subcategoryName)) {
        setSelectedSub(subcategoryName);
        setFilteredProducts(
          allProducts.filter((p) => p.subcategoryId?.name === subcategoryName)
        );
      }
    } catch (err) {
      console.error("Products fetch error:", err.response?.data || err.message);
      Alert.alert("Error", "Products load nahi hue bhai!");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubFilter = (filter) => {
    ...
    if (filter === "All") {
      ...
      setFilteredProducts(
        products.filter((p) => p.subcategoryId?.name === filter)
      );
    }
  };
          ...
          <View style={styles.cardInfo}>
            ...
            <View style={styles.priceRow}>
              <View style={styles.qtyControl}>
                ...
                >
                  <Text
                    style={{ fontSize: 18, color: "#27ae60", fontWeight: "bold" }}
                  >
                    +
                  </Text>
                </TouchableOpacity>
            ...
            {isInCart ? (
              <TouchableOpacity
                style={[styles.addBtn, styles.goToCartBtn]}
              ...
              <TouchableOpacity
                style={[styles.addBtn, styles.addToCartBtn]}
                onPress={() => increaseQty(item._id)}
              >
          ...
          >
            {uniqueSubcategories.map((item) => (
              <TouchableOpacity
                ...
                style={[
                  styles.filterBtn,
                  selectedSub === item && styles.activeBtn,
                ]}
              >
                <Text
                  style={[
                    styles.filterText,
                    selectedSub === item && styles.activeText,
                  ]}
                >
                  {item}
                </Text>
